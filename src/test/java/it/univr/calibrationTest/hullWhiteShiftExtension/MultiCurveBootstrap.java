...
/*
 * This is code is taken from the Finmath test classes
 * originally written by Christian Fries
 */
public class MultiCurveBootstrap {

	public static AnalyticModel getCurves() throws SolverException, CloneNotSupportedException {
		...
		/*
		 * These are some handy function that allow us to
		 * automatize the construction of calibration instruments
		 */
		final BiFunction<String, Double, CalibrationSpec> deposit = 
				new BiFunction<String, Double, CalibrationSpec>() {
			@Override
			public CalibrationSpec apply(final String maturity, final Double rate) {
				final Schedule scheduleInterfaceRec = 
						ScheduleGenerator.createScheduleFromConventions(
								referenceDate, 2, "0D", maturity, "tenor", "act/360", 
								"first", "following", 
								new BusinessdayCalendarExcludingTARGETHolidays(), 0, 0);
				final Schedule scheduleInterfacePay = null;
				final double calibrationTime = 
						scheduleInterfaceRec.getPayment(scheduleInterfaceRec.getNumberOfPeriods()-1);
				final CalibrationSpec calibrationSpec = 
						new CalibratedCurves.CalibrationSpec("EUR-OIS-" + maturity, 
								"Deposit", scheduleInterfaceRec, "", rate, 
								"discount-EUR-OIS", 
								scheduleInterfacePay, null, 0.0, 
								null, "discount-EUR-OIS", calibrationTime);
				return calibrationSpec;
			}
		};

		final BiFunction<String, Double, CalibrationSpec> swapSingleCurve =
				new BiFunction<String, Double, CalibrationSpec>() {
			@Override
			public CalibrationSpec apply(final String maturity, final Double rate) {
				final Schedule scheduleInterfaceRec = 
						ScheduleGenerator.createScheduleFromConventions(
								referenceDate, 2, "0D", maturity, "annual", "act/360", 
								"first", "modified_following", 
								new BusinessdayCalendarExcludingTARGETHolidays(), 0, 1);
				final Schedule scheduleInterfacePay = 
						ScheduleGenerator.createScheduleFromConventions(
								referenceDate, 2, "0D", maturity, "annual", "act/360", 
								"first", "modified_following", 
								new BusinessdayCalendarExcludingTARGETHolidays(), 0, 1);
				...
				final CalibrationSpec calibrationSpec = 
						new CalibratedCurves.CalibrationSpec("EUR-OIS-" + maturity,
								"Swap", scheduleInterfaceRec, "forward-EUR-OIS", 0.0, 
								"discount-EUR-OIS", scheduleInterfacePay, "", rate, 
								"discount-EUR-OIS", "discount-EUR-OIS", calibrationTime);
				return calibrationSpec;
			}
		};


		final Function<String,BiFunction<String, Double, CalibrationSpec>> fra = 
				new Function<String, BiFunction<String, Double, CalibrationSpec>>() {
			@Override
			public BiFunction<String, Double, CalibrationSpec> apply(final String tenor) {
				return new BiFunction<String, Double, CalibrationSpec>() {
					...
					public CalibrationSpec apply(final String fixing, final Double rate) {
						final Schedule scheduleInterfaceRec = 
								ScheduleGenerator.createScheduleFromConventions(
										referenceDate, 2, fixing, tenor, "tenor", "act/360", 
										"first", "modified_following", 
										new BusinessdayCalendarExcludingTARGETHolidays(), 0, 0);
						final double calibrationTime = 
								scheduleInterfaceRec.getFixing(scheduleInterfaceRec.getNumberOfPeriods() - 1);
						final String curveName = "forward-EUR-" + tenor;
						final CalibrationSpec calibrationSpec = 
								new CalibratedCurves.CalibrationSpec("EUR-" + tenor + "-" + fixing, 
										"FRA", scheduleInterfaceRec, curveName, rate, 
										"discount-EUR-OIS", null, null, 0.0,
										null, curveName, calibrationTime);
						return calibrationSpec;
					}
				};
			}
		...
		final Function<String,BiFunction<String, Double, CalibrationSpec>> swap = 
				new Function<String, BiFunction<String, Double, CalibrationSpec>>() {
			@Override
			public BiFunction<String, Double, CalibrationSpec> apply(final String tenor) {
				return new BiFunction<String, Double, CalibrationSpec>() {
					...
					public CalibrationSpec apply(final String maturity, final Double rate) {
						...
						final Schedule scheduleInterfaceRec = 
								ScheduleGenerator.createScheduleFromConventions(
										referenceDate, 2, "0D", maturity, frequencyRec, "act/360", 
										"first", "following",
										new BusinessdayCalendarExcludingTARGETHolidays(), 0, 0);
						final Schedule scheduleInterfacePay = 
								ScheduleGenerator.createScheduleFromConventions(
										referenceDate, 2, "0D", maturity, "annual", "E30/360", 
										"first", "following", 
										new BusinessdayCalendarExcludingTARGETHolidays(), 0, 0);
						final double calibrationTime = 
								scheduleInterfaceRec.getFixing(scheduleInterfaceRec.getNumberOfPeriods() - 1);
						final String curveName = "forward-EUR-" + tenor;
						final CalibrationSpec calibrationSpec = 
								new CalibratedCurves.CalibrationSpec("EUR-" + tenor + maturity, 
										"Swap", scheduleInterfaceRec, curveName, 0.0, 
										"discount-EUR-OIS", scheduleInterfacePay, "", rate, 
										"discount-EUR-OIS", curveName, calibrationTime);
						return calibrationSpec;
					}
				};
			}
		...
		final BiFunction<String,String,BiFunction<String, Double, CalibrationSpec>> swapBasis = 
				new BiFunction<String, String, BiFunction<String, Double, CalibrationSpec>>() {
			@Override
			public BiFunction<String, Double, CalibrationSpec> 
									apply(final String tenorRec, final String tenorPay) {
				return new BiFunction<String, Double, CalibrationSpec>() {
					@Override
					public CalibrationSpec apply(final String maturity, final Double rate) {
						...
						final Schedule scheduleInterfacePay = 
								ScheduleGenerator.createScheduleFromConventions(
										referenceDate, 2, "0D", maturity, frequencyPay, "act/360", 
										"first", "following", 
										new BusinessdayCalendarExcludingTARGETHolidays(), 0, 0);
						final double calibrationTime = 
								scheduleInterfaceRec.getFixing(scheduleInterfaceRec.getNumberOfPeriods() - 1);
						...
						final CalibrationSpec calibrationSpec = 
								new CalibratedCurves.CalibrationSpec(
										"EUR-" + tenorRec + "-" + tenorPay + maturity, 
										"Swap", scheduleInterfaceRec, curveNameRec, 0.0, 
										"discount-EUR-OIS", scheduleInterfacePay, curveNamePay, rate, 
										"discount-EUR-OIS", curveNameRec, calibrationTime);
						return calibrationSpec;
					}
				};
			}
		...
		final DiscountCurveInterpolation discountCurveOIS = 
				DiscountCurveInterpolation.createDiscountCurveFromDiscountFactors(
						"discount-EUR-OIS", referenceDate, times, discountFactors, 
						isParameter, InterpolationMethod.CUBIC_SPLINE, 
						ExtrapolationMethod.CONSTANT, InterpolationEntity.LOG_OF_VALUE);
		final ForwardCurve forwardCurveOIS = new ForwardCurveFromDiscountCurve(
				"forward-EUR-OIS", "discount-EUR-OIS", referenceDate, "3M");
		final ForwardCurve forwardCurve3M = 
				new ForwardCurveInterpolation("forward-EUR-3M", referenceDate, "3M", 
						new BusinessdayCalendarExcludingTARGETHolidays(),
						DateRollConvention.FOLLOWING, CurveInterpolation.InterpolationMethod.LINEAR, 
						CurveInterpolation.ExtrapolationMethod.CONSTANT, 
						CurveInterpolation.InterpolationEntity.VALUE,
						ForwardCurveInterpolation.InterpolationEntityForward.FORWARD, "discount-EUR-OIS");
		final ForwardCurve forwardCurve6M = 
				new ForwardCurveInterpolation("forward-EUR-6M", referenceDate, "6M", 
						new BusinessdayCalendarExcludingTARGETHolidays(), 
						DateRollConvention.FOLLOWING, CurveInterpolation.InterpolationMethod.LINEAR, 
						CurveInterpolation.ExtrapolationMethod.CONSTANT, 
						CurveInterpolation.InterpolationEntity.VALUE,
		...
		final AnalyticModelFromCurvesAndVols forwardCurveModel = 
				new AnalyticModelFromCurvesAndVols(new Curve[] { discountCurveOIS, 
						forwardCurveOIS, forwardCurve3M, forwardCurve6M });
		 ...
		 * Calibrate
		 */
		final CalibratedCurves calibratedCurves = 
				new CalibratedCurves(calibrationSpecs.toArray(
						new CalibrationSpec[calibrationSpecs.size()]), forwardCurveModel, 1E-15);
